...
plot_dat0 <- plot_dat %>% 
  filter(pEx == "pEx0", Exposure %in% c(0,2)) %>% 
...
plot_dat1 <- plot_dat %>% 
  filter(pEx == "pEx1", Exposure %in% c(1,2)) %>% 
  mutate(Exposure = factor(Exposure))
...
ggsave(here("figures", "ps_overlap_plot.png"),
...
modelRD_ex <- lm(wt82_71 ~ relevel(factor(exercise),
                                    ref = "2"),
                 weight = sw_exercise,
                 data = nhefs)

coeftest(modelRD_ex, vcov. = vcovHC(modelRD_ex, "HC3"))

coefci(modelRD_ex, vcov. = vcovHC(modelRD_ex, "HC3"))
...
boot_func <- function(data, index){
  ...
  return(summary(modelRD_ex)$coefficients[2:3, 1])
}
